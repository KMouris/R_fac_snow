...
"""Boolean variables to determine which modules to run and which input parameters to include: 
    - run_pt_manipulation: 'True' to run precipitation and temperature manipulation code to obtain the .csv files which 
    are used in "rain_snow_rasters" to determine precipitation and snow rasters. 'False' to directly input the .csv 
    ...
    - run_snow_cover: 'True' to run snow_cover code, 'False' to directly input the binary snow raster, to calculate 
    areas of snow from satellite images.
...
Files needed regardless of which codes to run: 
    ...
    (obtain projection, extension and raster resolution). It is usually DEM raster file.
    ...
    - start_date, end_date: in YYYYMM format, determines the time interval to analyze for. If they are the same, only 
...
If run_pt_manipulation = 'False' (AND run_rain_snow_rasters = True)
    - PT_path: Path of folder with precipitation and Temperature data, for each cell, for time span to analyze. 
    Each file is a .csv file corresponding to the row_column of each cell. Each row in the array is a different 
    year-month-day-hour. The format of the column is year-month-day-hour-minute-second-Precipitation-Temperature-Row
    -Column.
...
If run_rain_snow_rasters = True: 
    - T_snow: Minimum temperature threshold to detect snowfall, very low value means the whole precipitation is considered as rain.
"""
T_snow = 0
...
If "run_snow_cover" = True: 
    ...
    - image_list: Name of bands to merge, clip and resample. The name in the list must correspond to the final suffix of 
        the satellite image name. They must be in the following order: band02, band03, band04, band11, TCI.
...
- statistic_param: Definition of statistical parameter to be plotted ('min', 'mean', 'max', 'range', 
'sum', 'coverage') """
