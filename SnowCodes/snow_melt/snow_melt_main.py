...
def filter_raster_lists(list):
    ...
    new_list = []
    for elem in list:
        data_manager = DataManagement(path=r'' + os.path.abspath('../Results'), filename=elem)
    ...
    n_months = (end_date.year - start_date.year) * 12 + end_date.month - start_date.month + 1  # months to analyze
...
def main():
    ...
    j = 0
    for file in snow_mm:
        check_data(snow_mm[j], snow_cover[j], snow_mm_paths[j], snow_cover_paths[j], snow_mm_paths, snow_cover_paths)
        j += 1
