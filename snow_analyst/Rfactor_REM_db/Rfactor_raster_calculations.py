"""Calls functions needed in the "RFactor_main" python program, located in the same folder. The functions
...
def check_input_rasters(raster1, raster2):
    """Checks if 2 input rasters (e.g. precipitation raster and fEL raster) have the same cell size, and number of
    ...
    # Get Geotransform Data: (Top left corner X, cell size, 0, Top left corner Y, 0,
    gt_1 = raster.GetGeoTransform()
    # -cell size)
...
def get_raster_data(raster_path):
    ...
    # Get Geotransform Data: Coordinate upper left X, cell size, 0, Coord. upper left Y,
...
def raster_to_array(raster_path, mask=True):
    ...
    # Get NoData value, since all input rasters could have different values
    no_data = np.float32(band.GetNoDataValue())
    #                                                   and assign a numpy type variable
    # Save band info as array and assign the same data type as no_data to
    array = np.float32(band.ReadAsArray())
    #                                                   avoid inequalities

    if mask:
        # Create a masked array from the input data
...
def save_raster(array, output_path, gt, proj):
    ...
    outrs = driver.Create(
        output_path, xsize=array.shape[1], ysize=array.shape[0], bands=1, eType=gdal.GDT_Float32)
